...
valid_azurerm_mariadb_ssl_enforcement_enabled(resource) if {
    resource.values.ssl_enforcement_enabled == true
}

deny_mariadb_ssl_enforcement_enabled contains reason if {
    resource := data.utils.resource(input, "azurerm_mariadb_server")[_]
    not valid_azurerm_mariadb_ssl_enforcement_enabled(resource)
