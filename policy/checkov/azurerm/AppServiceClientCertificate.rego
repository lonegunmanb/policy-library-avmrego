...
valid_azurerm_app_service_client_certificate(resource) if {
    resource.values.client_cert_enabled == true
}

valid_azurerm_linux_web_app_client_certificate(resource) if {
    resource.values.client_certificate_enabled == true
}

valid_azurerm_windows_web_app_client_certificate(resource) if {
    resource.values.client_certificate_enabled == true
}

deny_CKV_AZURE_17 contains reason if {
    resource := data.utils.resource(input, "azurerm_app_service")[_]
    not valid_azurerm_app_service_client_certificate(resource)
...
deny_CKV_AZURE_17 contains reason if {
    ...
    not valid_azurerm_linux_web_app_client_certificate(resource)
...
deny_CKV_AZURE_17 contains reason if {
    ...
    not valid_azurerm_windows_web_app_client_certificate(resource)
