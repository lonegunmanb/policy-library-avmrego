...
deny_CKV_AZURE_7 contains reason if {
    ...
    reason := sprintf("checkov/CKV_AZURE_7: Ensure AKS cluster has Network Policy configured %s", [resource.address])
    reason := sprintf("%s https://github.com/bridgecrewio/checkov/blob/main/checkov/terraform/checks/resource/azure/AKSNetworkPolicy.py", [reason])
}
