...
valid_azurerm_linux_web_app_detailed_error_messages_enabled(resource) if {
    resource.type == "azurerm_linux_web_app"
    resource.values.logs[0].detailed_error_messages == true
}

valid_azurerm_windows_web_app_detailed_error_messages_enabled(resource) if {
    resource.type == "azurerm_windows_web_app"
    resource.values.logs[0].detailed_error_messages == true
}

deny_app_service_detailed_error_messages_enabled contains reason if {
...
deny_linux_web_app_detailed_error_messages_enabled contains reason if {
    resource := data.utils.resource(input, "azurerm_linux_web_app")[_]
    not valid_azurerm_linux_web_app_detailed_error_messages_enabled(resource)
...
deny_windows_web_app_detailed_error_messages_enabled contains reason if {
    resource := data.utils.resource(input, "azurerm_windows_web_app")[_]
    not valid_azurerm_windows_web_app_detailed_error_messages_enabled(resource)
