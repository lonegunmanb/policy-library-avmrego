...
valid_azurerm_mariadb_geo_backup_enabled(resource) if {
    resource.values.geo_redundant_backup_enabled == true
}

deny_mariadb_geo_backup_enabled contains reason if {
    resource := data.utils.resource(input, "azurerm_mariadb_server")[_]
    not valid_azurerm_mariadb_geo_backup_enabled(resource)
